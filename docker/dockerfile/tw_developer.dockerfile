...
# docker run -it --name tw_development -p 3256:22 -v ~/.config/dockerHost:/home/host:rw herrwinfried/dev_env:opensuse_tumbleweed
...
RUN zypper --gpg-auto-import-keys in -y -l which shadow hostname zsh fish bash-completion iputils net-tools net-tools-deprecated openssh
...
RUN dnf --nogpgcheck makecache -y
...
# DEVELOPMENT
RUN dnf --nogpgcheck install --setopt=install_weak_deps=False -y patterns-devel-base-devel_basis patterns-devel-C-C++-devel_C_C++ 

RUN dnf --nogpgcheck install --setopt=install_weak_deps=False -y build gdb ninja llvm-clang gcc gcc-c++ cmake cmake-full extra-cmake-modules

RUN dnf --nogpgcheck install --setopt=install_weak_deps=False -y rust cargo

RUN dnf --nogpgcheck install --setopt=install_weak_deps=False -y dotnet-sdk-7.0

RUN dnf --nogpgcheck install -y patterns-devel-mono-devel_mono

RUN dnf --nogpgcheck install --setopt=install_weak_deps=False -y patterns-devel-base-devel_rpm_build

RUN dnf --nogpgcheck install -y patterns-kde-devel_qt6 
RUN dnf --nogpgcheck install -y patterns-kde-devel_qt5 
RUN zypper --gpg-auto-import-keys install -y --recommends patterns-kde-devel_kde_frameworks

RUN dnf --nogpgcheck install --setopt=install_weak_deps=False -y rsync gtk3-devel java-20-openjdk nodejs-default npm-default python311
# CONFIG
...
RUN su -s /bin/fish -l ${user} -c 'fish_config theme choose "Dracula Official"'
