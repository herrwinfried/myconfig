...
SUDO su -c "echo \"test -s /etc/profile.d/distroboxenvimage.sh && . /etc/profile.d/distroboxenvimage.sh\" | tee -a /etc/bashrc"

SUDO su -c "echo \"test -s /etc/profile.d/distroboxenvimage.sh && . /etc/profile.d/distroboxenvimage.sh\" | tee -a /etc/zshrc"
...
SUDO su -c "pwsh -c $ScriptFolder1/dotfiles/temp/containervariable.ps1"
fi