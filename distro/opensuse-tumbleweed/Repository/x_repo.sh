...
sudo zypper ar --priority 50 -d https://download.opensuse.org/repositories/home:/luke_nukem:/asus/openSUSE_Tumbleweed/ asus-linux


sudo rpm --import https://download.copr.fedorainfracloud.org/results/lukenukem/asus-linux/pubkey.gpg

cat > asus-linux-fedora.repo << "EOF"
[asus-linux-FEDORA]
enabled=1
autorefresh=1
...
sudo zypper --gpg-auto-import-keys addrepo --refresh --repo asus-linux-fedora.repo
...
sudo zypper --gpg-auto-import-keys addrepo http://dl.google.com/linux/chrome/rpm/stable/x86_64 Google-Chrome
}

function edgerepo {
sudo rpm --import https://packages.microsoft.com/keys/microsoft.asc
sudo zypper --gpg-auto-import-keys addrepo https://packages.microsoft.com/yumrepos/edge microsoft-edge
}
