...
function Update-AliasFiles {
    ...
    Invoke-WebRequest -Uri "https://raw.githubusercontent.com/herrwinfried/myconfig/linux/data/superuser/.alias.ps1" -OutFile "$env:HOME/.alias.ps1" -Quiet
    Invoke-WebRequest -Uri "https://raw.githubusercontent.com/herrwinfried/myconfig/linux/data/superuser/.alias" -OutFile "$env:HOME/.alias" -Quiet
...
if ((Get-Command oh-my-posh -ErrorAction SilentlyContinue) -and (Test-Path $POSH_THEME)) {
    ...
    function Update-OhMyPoshTheme {
        ...
        Invoke-WebRequest -Uri "https://raw.githubusercontent.com/herrwinfried/myconfig/linux/data/superuser/.poshthemes/default.omp.json" -OutFile $POSH_THEME -Quiet
